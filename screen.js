(function () {
  ...
  window.Screen = function (channel) {
    var signaler,
      self = this;
    this.channel = channel || getUrlVars()["id"];
    ...
    function initSignaler(roomid) {
      signaler = new Signaler(
        self,
        (roomid && roomid.length ? roomid : null) || self.channel
      );
    }

    function captureUserMedia(callback, extensionAvailable) {
      if (isEdge) {
        navigator.getDisplayMedia({ video: true }).then(
          (stream) => {
      ...
      getScreenId(function (error, sourceId, screen_constraints) {
        ...
        console.log(
          "screen_constraints",
          JSON.stringify(screen_constraints, null, "\t")
        );
        navigator.mediaDevices
          ...
          .catch(function (error) {
            if (
              ...
              alert(
                "Screen capturing is either denied or not supported. Please install chrome extension for screen capturing or run chrome with command-line flag: --enable-usermedia-screen-capturing"
              );
    ...
    createOffer: function (config) {
      ...
      peer.oniceconnectionstatechange = peer.onsignalingstatechange = function () {
        if (
          ...
          peer.iceConnectionState.search(/disconnected|closed|failed/gi) !== -1
        ) {
          if (peers[config.to]) {
  ...
  var Answer = {
    createAnswer: function (config) {
      ...
      peer
        .setRemoteDescription(new RTCSessionDescription(config.sdp))
        .then(function () {
          peer
            .createAnswer(answerConstraints)
            .then(function (sdp) {
              sdp.sdp = setBandwidth(sdp.sdp);
              peer
